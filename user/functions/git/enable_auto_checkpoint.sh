...
# Function to enable cleanup background worker
enable_cleanup_worker() {
    log_message "(BEGIN ENABLE_CLEANUP_WORKER)"
    
    # Only run in statusbar mode (when no other mode flags are set)
    if [ "$navbar" = true ] || [ "$progressbar" = true ] || [ "$droplet" = true ]; then
        log_message "Skipping enable_cleanup_worker - not in statusbar mode (navbar=$navbar, progressbar=$progressbar, droplet=$droplet)"
        return 0
    fi
    
    log_message "Running enable_cleanup_worker in statusbar mode"
    ...
    # Define the plist file path
    PLIST_FILE="$LAUNCH_AGENT_DIR/com.unflab.cleanup.plist"
...
<plist version="1.0">
    <dict>
        <key>Label</key>
        <string>com.unflab.cleanup</string>

        <key>ProgramArguments</key>
        <array>
            <string>'"$BASH_SCRIPT_PATH"'</string>
            <string>cleanup_check</string>
            <string>--silent</string>
        </array>
        ...
        <key>StandardOutPath</key>
        <string>/tmp/unflab-cleanup.out</string>
        <key>StandardErrorPath</key>
        <string>/tmp/unflab-cleanup.err</string>
    </dict>
</plist>'
    ...
    # Check if the LaunchAgent exists and compare contents
    if [ -f "$PLIST_FILE" ]; then
        log_message "Cleanup plist file found."
        CURRENT_CONTENT=$(cat "$PLIST_FILE")
        if [ "$CURRENT_CONTENT" != "$PLIST_CONTENT" ]; then
            log_message "Updating cleanup plist file."
            echo "$PLIST_CONTENT" > "$PLIST_FILE"
            log_message "Reloading cleanup service."
            launchctl unload "$PLIST_FILE" 2>/dev/null
            launchctl load "$PLIST_FILE" 2>/dev/null
            made_changes=true
        else
            log_message "Cleanup service is up to date."
        fi
    else
        log_message "Cleanup plist file not found, creating new file."
        echo "$PLIST_CONTENT" > "$PLIST_FILE"
        made_changes=true
    fi

    # Check if the service is loaded
    log_message "Checking if cleanup service is loaded."
    if ! launchctl list | grep -q "com.unflab.cleanup"; then
        log_message "Cleanup service not loaded, loading now"
        launchctl load "$PLIST_FILE" 2>/dev/null
        made_changes=true
    fi
    
    if [ $? -eq 0 ]; then
        log_message "Cleanup service enabled successfully"
        if [ "$made_changes" = true ]; then
            log_message "Cleanup service enabled (runs daily)"
        fi
    else
        log_message "Failed to enable cleanup service"
        return 1
    fi
    
    log_message "(END ENABLE_CLEANUP_WORKER)"
}

disable_cleanup_worker() {
    PLIST_FILE="$HOME/Library/LaunchAgents/com.unflab.cleanup.plist"
    
    # Unload the LaunchAgent if it exists
    if [ -f "$PLIST_FILE" ]; then
        launchctl unload "$PLIST_FILE" 2>/dev/null
        rm "$PLIST_FILE"
        log_message "Cleanup service disabled"
    fi
}
